...
setwd("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/sun_mr//output/")
...
#SUHRE
setwd("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/suhr_mr/outputs/")
...
setwd("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/breth_mr/output/")
...
setwd("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/folk_mr/output/")
...
setwd("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/sliz_mr/output/")
...
setwd("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/wood_mr/output/")
...
setwd("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/infl_mr/output/")
...
setwd("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/scal_mr/output/")
...
setwd("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/hill_mr/output/")
...
setwd("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/hogl_mr/output/")
...
setwd("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/enroth_mr/output/")
...
```{r echo=FALSE}
total_tests <- sum(ahol_n$n, breth_n$n, folk_n$n, scal_n$n, sliz_n$n, suhre_n$n, sun_n$n, wood_n$n, hill_n$n, hogl_n$n, enroth_n$n)
...
mr_df_all <- rbind(breth_mr, folk_mr, sliz_mr, suhre_mr, sun_mr, wood_mr, ahol_mr, scal_mr, hill_mr, hogl_mr, enroth_mr)
covid <- str_count(mr_df_all$Exposure, "Very_severe_COVID")
covid[is.na(covid)] <- 0
sum(covid)
```
...
```{r}
markers <- mr_df_all[- grep("Very_severe_COVID", mr_df_all$Exposure),]
nrow(markers)
```

# Bind all data frames
```{r}
mr_df <- rbind(breth_mr, folk_mr, sliz_mr, suhre_mr, sun_mr, wood_mr, ahol_mr, scal_mr, hill_mr, hogl_mr, enroth_mr)
```
...
Covid_outcome <- mr_df_all[!grepl("Very_severe_COVID", mr_df_all$Exposure),]
...
Covid_exposure <- mr_df_all[!grepl("Very", mr_df_all$Outcome),]
...
mr_df_filtered[1,] <- gsub("GCNT4", "GCNT4_Sun", mr_df_filtered[1,])
mr_df_filtered[2,] <- gsub("FAM96A", "FAM96A_Sun",mr_df_filtered[2,])
mr_df_filtered[3,] <- gsub("RAB14", "RAB14_Sun",mr_df_filtered[3,])
mr_df_filtered[4,] <- gsub("C1GALT1C1", "C1GALT1C1_Sun",mr_df_filtered[4,])
mr_df_filtered[5,] <- gsub("CD207", "CD207_Sun",mr_df_filtered[5,])
mr_df_filtered[6,] <- gsub("ABO", "ABO_Sun",mr_df_filtered[6,])
mr_df_filtered[7,] <- gsub("sEselectin", "SELE_Sliz",mr_df_filtered[7,])
mr_df_filtered[8,] <- gsub("SELL", "SELL_Sun",mr_df_filtered[8,])
mr_df_filtered[9,] <- gsub("SELE", "SELE_Scal",mr_df_filtered[9,])
mr_df_filtered[10,] <- gsub("sICAM1", "sICAM1_Sliz",mr_df_filtered[10,])
mr_df_filtered[11,] <- gsub("SELE", "SELE_Folk",mr_df_filtered[11,])
mr_df_filtered[12,] <- gsub("SELE", "SELE_Breth",mr_df_filtered[12,])
mr_df_filtered[13,] <- gsub("PECAM1", "PECAM1_Folk",mr_df_filtered[13,])
mr_df_filtered[14,] <- gsub("NEP", "NEP_Hill",mr_df_filtered[14,])
mr_df_filtered[15,] <- gsub("PECAM-1", "PECAM1_Scal",mr_df_filtered[15,])
...
mr_df_filtered <- mr_df_filtered[!grepl("Very_severe", mr_df_filtered$Exposure),]

p1 <- ggplot(mr_df_filtered, aes(x=reorder(as.factor(Exposure), Beta_exponent) , y=Beta_exponent)) + 
    geom_boxplot(fill="slateblue") + coord_flip() + geom_errorbar(aes(ymax = UpperCI, ymin = LowerCI))+ ggtitle("Outcome - Respiratory Support/Death Covid-19") +
...
sEselectin <- sliz_mr[grep("sEselectin", sliz_mr$Outcome),]
SELL <- sun_mr[grep("SELL", sun_mr$Outcome),]
SELE <- scal_mr[grep("SELE", scal_mr$Outcome),]
sICAM1 <- sliz_mr[grep("sICAM1", sliz_mr$Outcome),]
SELE_1 <- folk_mr[grep("SELE", folk_mr$Outcome),]
SELE_2 <- breth_mr[grep("SELE", breth_mr$Outcome),]
PECAM1 <- folk_mr[grep("PECAM1", folk_mr$Outcome),]
...
covid_exposure[2,] <- gsub("RAB14", "RAB14_Sun",covid_exposure[2,])
covid_exposure[3,] <- gsub("C1GALT1C1", "C1GALT1C1_Sun",covid_exposure[3,])
covid_exposure[4,] <- gsub("CD207", "CD207_Sun",covid_exposure[4,])
covid_exposure[5,] <- gsub("ABO", "ABO_Sun",covid_exposure[5,])
covid_exposure[6,] <- gsub("sEselectin", "SELE_Sliz",covid_exposure[6,])
covid_exposure[7,] <- gsub("SELL", "SELL_Sun",covid_exposure[7,])
covid_exposure[8,] <- gsub("SELE.txt", "SELE_Scal",covid_exposure[8,])
covid_exposure[9,] <- gsub("sICAM1", "sICAM1_Sliz",covid_exposure[9,])
covid_exposure[10,] <- gsub("SELE", "SELE_Folk",covid_exposure[10,])
covid_exposure[11,] <- gsub("SELE", "SELE_Breth",covid_exposure[11,])
covid_exposure[12,] <- gsub("PECAM1", "PECAM1_Folk",covid_exposure[12,])
covid_exposure[13,] <- gsub("PECAM-1.txt", "PECAM1_Scal",covid_exposure[13,])
...
p2 <- ggplot(covid_exposure_sorted, aes(x=reorder(as.factor(Outcome), Beta_exponent) , y=Beta_exponent)) + 
    geom_boxplot(fill="slateblue") + coord_flip() + geom_errorbar(aes(ymax = UCI, ymin = LCI))+ ggtitle("Exposure - Respiratory Support/Death Covid-19") +
...
ggsave(path = "/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/", filename = "Figure2", plot = figure, device='tiff', dpi=700)
...
#read in results
GCNT4 <- fread("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/sun_mr/output/SUN1187.txt.gsmr", verbose = T, data.table = F)
FAM96A <- fread("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/sun_mr/output/SUN1052.txt.gsmr", verbose = T, data.table = F)
RAB14 <- fread("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/sun_mr/output/SUN2468.txt.gsmr", verbose = T, data.table = F)
C1GALT1C1 <- fread("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/sun_mr/output/SUN0294.txt.gsmr", verbose = T, data.table = F)
CD207 <- fread("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/sun_mr/output/SUN0426.txt.gsmr", verbose = T, data.table = F)
ABO <- fread("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/sun_mr/output/SUN0010.txt.gsmr", verbose = T, data.table = F)
sEselectin <- fread("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/sliz_mr/output/sEselectin.txt.gsmr", verbose = T, data.table = F)
SELL <- fread("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/sun_mr/output/SUN2663.txt.gsmr", verbose = T, data.table = F)
SELE <- fread("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/scal_mr/output/SELE.txt.gsmr", verbose = T, data.table = F)
sICAM1 <- fread("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/sliz_mr/output/sICAM1.txt.gsmr", verbose = T, data.table = F)
SELE_1 <- fread("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/folk_mr/output/SELE.txt.gsmr", verbose = T, data.table = F)
SELE_2 <- fread("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/breth_mr/output/BRM189.txt.gsmr", verbose = T, data.table = F)
PECAM1 <- fread("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/folk_mr/output/PECAM1.txt.gsmr", verbose = T, data.table = F)
PECAM_1 <- fread("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/scal_mr/output/PECAM-1.txt.gsmr", verbose = T, data.table = F)

#get SNP data
kable(GCNT4)
GCNT4_chart = read_gsmr_data("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/sun_mr/output/SUN1187.txt.eff_plot.gz")
plot_gsmr_effect(GCNT4_chart, "GCNT4.10842.7.3", "Very_severe_COVID", colors()[75])  
gsmr_snp_effect(GCNT4_chart, "GCNT4.10842.7.3", "Very_severe_COVID") %>%
  kable()

kable(FAM96A)
FAM96A_chart = read_gsmr_data("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/sun_mr/output/SUN1052.txt.eff_plot.gz")
plot_gsmr_effect(FAM96A_chart, "FAM96A.8787.21.3", "Very_severe_COVID", colors()[75])  
gsmr_snp_effect(FAM96A_chart, "FAM96A.8787.21.3", "Very_severe_COVID") %>%
  kable()

kable(RAB14)
RAB14_chart = read_gsmr_data("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/sun_mr/output/SUN2468.txt.eff_plot.gz")
plot_gsmr_effect(RAB14_chart, "RAB14.14283.12.3", "Very_severe_COVID", colors()[75])  
gsmr_snp_effect(RAB14_chart, "RAB14.14283.12.3", "Very_severe_COVID") %>%
  kable()

kable(C1GALT1C1)
C1GALT1C1_chart = read_gsmr_data("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/sun_mr/output/SUN0294.txt.eff_plot.gz")
plot_gsmr_effect(C1GALT1C1_chart, "C1GALT1C1.5735.54.3", "Very_severe_COVID", colors()[75])  
gsmr_snp_effect(C1GALT1C1_chart, "C1GALT1C1.5735.54.3", "Very_severe_COVID") %>%
  kable()

kable(CD207)
CD207_chart = read_gsmr_data("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/sun_mr/output/SUN0426.txt.eff_plot.gz")
plot_gsmr_effect(CD207_chart, "CD207.3361.26.2", "Very_severe_COVID", colors()[75])  
gsmr_snp_effect(CD207_chart, "CD207.3361.26.2", "Very_severe_COVID") %>%
  kable()

kable(ABO)
ABO_chart = read_gsmr_data("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/sun_mr/output/SUN0010.txt.eff_plot.gz")
plot_gsmr_effect(ABO_chart, "ABO.9253.52.3", "Very_severe_COVID", colors()[75])  
gsmr_snp_effect(ABO_chart, "ABO.9253.52.3", "Very_severe_COVID") %>%
  kable()

kable(sEselectin)
sEselectin_chart = read_gsmr_data("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/sliz_mr/output/sEselectin.txt.eff_plot.gz")
plot_gsmr_effect(sEselectin_chart, "sEselectin", "Very_severe_COVID", colors()[75])  
gsmr_snp_effect(sEselectin_chart, "sEselectin", "Very_severe_COVID") %>%
  kable()

kable(SELL)
SELL_chart = read_gsmr_data("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/sun_mr/output/SUN2663.txt.eff_plot.gz")
plot_gsmr_effect(SELL_chart, "SELL.4831.4.2", "Very_severe_COVID", colors()[75])  
gsmr_snp_effect(SELL_chart, "SELL.4831.4.2", "Very_severe_COVID") %>%
  kable()

kable(SELE)
SELE_chart = read_gsmr_data("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/scal_mr/output/SELE.txt.eff_plot.gz")
plot_gsmr_effect(SELE_chart, "SELE.txt", "Very_severe_COVID", colors()[75])  
gsmr_snp_effect(SELE_chart, "SELE.txt", "Very_severe_COVID") %>%
  kable()

kable(sICAM1)
sICAM1_chart = read_gsmr_data("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/sliz_mr/output/sICAM1.txt.eff_plot.gz")
plot_gsmr_effect(sICAM1_chart, "sICAM1", "Very_severe_COVID", colors()[75])  
gsmr_snp_effect(sICAM1_chart, "sICAM1", "Very_severe_COVID") %>%
  kable()

kable(SELE_1)
SELE_1_chart = read_gsmr_data("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/folk_mr/output/SELE.txt.eff_plot.gz")
plot_gsmr_effect(SELE_1_chart, "SELE", "Very_severe_COVID", colors()[75])  
gsmr_snp_effect(SELE_1_chart, "SELE", "Very_severe_COVID") %>%
  kable()

kable(SELE_2)
SELE_2_chart = read_gsmr_data("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/breth_mr/output/BRM189.txt.eff_plot.gz")
plot_gsmr_effect(SELE_2_chart, "SELE", "Very_severe_COVID", colors()[75])  
gsmr_snp_effect(SELE_2_chart, "SELE", "Very_severe_COVID") %>%
  kable()

kable(PECAM1)
PECAM1_chart = read_gsmr_data("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/folk_mr/output/PECAM1.txt.eff_plot.gz")
plot_gsmr_effect(PECAM1_chart, "PECAM1", "Very_severe_COVID", colors()[75])  
gsmr_snp_effect(PECAM1_chart, "PECAM1", "Very_severe_COVID") %>%
  kable()

kable(PECAM_1)
PECAM_1_chart = read_gsmr_data("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/scal_mr/output/PECAM-1.txt.eff_plot.gz")
plot_gsmr_effect(PECAM_1_chart, "PECAM-1.txt", "Very_severe_COVID", colors()[75])  
gsmr_snp_effect(PECAM_1_chart, "PECAM-1.txt", "Very_severe_COVID") %>%
  kable()
```
...
sEselectin <- fread("/scratch/groups/ukbiobank/sumstats/cleaned/blood_biomarkers/SLI14.gz", verbose = T, data.table = F, fill=TRUE)
sEselectin <-  inner_join(sEselectin_snps, sEselectin)
...
sICAM1 <- fread("/scratch/groups/ukbiobank/sumstats/cleaned/blood_biomarkers/SLI15.gz", verbose = T, data.table = F, fill=TRUE)
sICAM1 <-  inner_join(sICAM1_snps, sICAM1)
...
SELE_2 <- fread("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/breth_mr/BRM189.txt", verbose = T, data.table = F, fill=TRUE)
SELE_2 <-  inner_join(SELE_2_snps, SELE_2)

PECAM1 <- fread("/scratch/groups/ukbiobank/sumstats/cleaned/blood_biomarkers/FOLK81.gz", verbose = T, data.table = F, fill=TRUE)
PECAM1 <-  inner_join(PECAM1_snps, PECAM1)
...
Severe_Covid <- fread("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/A2_all")
...
write.table(FAM96A, "FAM96A", col.names=T, row.names=F, quote=F,sep = "\t")
...
write.table(sEselectin,"sEselectin",row.names=FALSE,sep="\t", quote = FALSE)
write.table(SELL, "SELL", col.names=T, row.names=F, quote=F,sep = "\t")
write.table(SELE, "SELE", col.names=T, row.names=F, quote=F,sep = "\t")
write.table(sICAM1, "sICAM1", col.names=T, row.names=F, quote=F,sep = "\t")
...
write.table(PECAM1, "PECAM1", col.names=T, row.names=F, quote=F,sep = "\t")
...
exposure_GCNT4_dat <- read_exposure_data(
  filename = "/mnt/lustre/groups/ukbiobank/Edinburgh_Data/usr/alish/Covid19/MR/A2_All/GCNT4", 
  sep = "\t",                
  snp_col = "SNP",          
  beta_col = "BETA",         
  se_col = "SE",        
...
exposure_RAB14_dat <- read_exposure_data(
  filename = "/mnt/lustre/groups/ukbiobank/Edinburgh_Data/usr/alish/Covid19/MR/A2_All/RAB14", 
  sep = "\t",                
  snp_col = "SNP",          
  beta_col = "BETA",         
  se_col = "SE",        
...
exposure_C1GALT1C1_dat <- read_exposure_data(
  filename = "/mnt/lustre/groups/ukbiobank/Edinburgh_Data/usr/alish/Covid19/MR/A2_All/C1GALT1C1", 
  sep = "\t",                
  snp_col = "SNP",          
  beta_col = "BETA",         
  se_col = "SE",        
...
exposure_CD207_dat <- read_exposure_data(
  filename = "/mnt/lustre/groups/ukbiobank/Edinburgh_Data/usr/alish/Covid19/MR/A2_All/CD207", 
  sep = "\t",                
  snp_col = "SNP",          
  beta_col = "BETA",         
  se_col = "SE",        
...
exposure_ABO_dat <- read_exposure_data(
  filename = "/mnt/lustre/groups/ukbiobank/Edinburgh_Data/usr/alish/Covid19/MR/A2_All/ABO", 
  sep = "\t",                
  snp_col = "SNP",          
  beta_col = "BETA",         
  se_col = "SE",        
...
exposure_sEselectin_dat <- read_exposure_data(
  filename = "/mnt/lustre/groups/ukbiobank/Edinburgh_Data/usr/alish/Covid19/MR/A2_All/sEselectin", 
  sep = "\t",                
  snp_col = "SNP",          
  beta_col = "BETA",         
  se_col = "SE",        
  ...
  samplesize_col = "N",
  pval_col = "P") 
exposure_sEselectin_dat$exposure="sEselectin"

exposure_SELL_dat <- read_exposure_data(
  filename = "/mnt/lustre/groups/ukbiobank/Edinburgh_Data/usr/alish/Covid19/MR/A2_All/SELL", 
  sep = "\t",                
  snp_col = "SNP",          
  beta_col = "BETA",         
  se_col = "SE",        
...
exposure_SELE_dat <- read_exposure_data(
  filename = "/mnt/lustre/groups/ukbiobank/Edinburgh_Data/usr/alish/Covid19/MR/A2_All/SELE", 
  sep = "\t",                
  snp_col = "SNP",          
  beta_col = "BETA",         
  se_col = "SE",        
...
exposure_sICAM1_dat <- read_exposure_data(
  filename = "/mnt/lustre/groups/ukbiobank/Edinburgh_Data/usr/alish/Covid19/MR/A2_All/sICAM1", 
  sep = "\t",                
  snp_col = "SNP",          
  beta_col = "BETA",         
  se_col = "SE",        
  ...
  samplesize_col = "N",
  pval_col = "P") 
exposure_sICAM1_dat$exposure="sICAM1"

exposure_SELE_1_dat <- read_exposure_data(
  filename = "/mnt/lustre/groups/ukbiobank/Edinburgh_Data/usr/alish/Covid19/MR/A2_All/SELE_1", 
  sep = "\t",                
  snp_col = "SNP",          
  beta_col = "BETA",         
  se_col = "SE",        
  ...
  samplesize_col = "N",
  pval_col = "P") 
exposure_SELE_1_dat$exposure="SELE_1"

exposure_SELE_2_dat <- read_exposure_data(
  filename = "/mnt/lustre/groups/ukbiobank/Edinburgh_Data/usr/alish/Covid19/MR/A2_All/SELE_2", 
  sep = "\t",                
  snp_col = "SNP",          
  beta_col = "BETA",         
  se_col = "SE",        
  ...
  samplesize_col = "N",
  pval_col = "P") 
exposure_SELE_2_dat$exposure="SELE_2"

exposure_PECAM1_dat <- read_exposure_data(
  filename = "/mnt/lustre/groups/ukbiobank/Edinburgh_Data/usr/alish/Covid19/MR/A2_All/PECAM1", 
  sep = "\t",                
  snp_col = "SNP",          
  beta_col = "BETA",         
  se_col = "SE",        
  ...
  samplesize_col = "N",
  pval_col = "P") 
exposure_PECAM1_dat$exposure="PECAM1"

exposure_PECAM_1_dat <- read_exposure_data(
  filename = "/mnt/lustre/groups/ukbiobank/Edinburgh_Data/usr/alish/Covid19/MR/A2_All/PECAM_1", 
  sep = "\t",                
  snp_col = "SNP",          
  beta_col = "BETA",         
  se_col = "SE",        
...
outcome_Severe_Covid_dat <- read_outcome_data(
  filename = "/mnt/lustre/groups/ukbiobank/Edinburgh_Data/usr/alish/Covid19/MR/A2_All/Severe_Covid", 
  sep = "\t",                
  snp_col = "SNP",          
  beta_col = "BETA",         
  se_col = "SE",        
...
dataMR_sEselectintoCovid <- harmonise_data(exposure_dat = exposure_sEselectin_dat, outcome_dat = outcome_Severe_Covid_dat, action=2)
...
dataMR_sICAM1toCovid <- harmonise_data(exposure_dat = exposure_sICAM1_dat, outcome_dat = outcome_Severe_Covid_dat, action=2)
...
dataMR_PECAM1toCovid <- harmonise_data(exposure_dat = exposure_PECAM1_dat, outcome_dat = outcome_Severe_Covid_dat, action=2)
...
dataMR_sEselectintoCovid_keep=subset(dataMR_sEselectintoCovid, mr_keep==TRUE)
...
dataMR_sICAM1toCovid_keep=subset(dataMR_sICAM1toCovid, mr_keep==TRUE)
...
dataMR_PECAM1toCovid_keep=subset(dataMR_PECAM1toCovid, mr_keep==TRUE)
...
dataMR_sEselectintoCovid_keep$BetaXG<-dataMR_sEselectintoCovid_keep$beta.exposure
dataMR_sEselectintoCovid_keep$seBetaXG<-dataMR_sEselectintoCovid_keep$se.exposure
BetaXG   = dataMR_sEselectintoCovid_keep$BetaXG
seBetaXG = dataMR_sEselectintoCovid_keep$seBetaXG 
...
#PECAM1_Folk
#Rename required columns
dataMR_PECAM1toCovid_keep$BetaXG<-dataMR_PECAM1toCovid_keep$beta.exposure
dataMR_PECAM1toCovid_keep$seBetaXG<-dataMR_PECAM1toCovid_keep$se.exposure
BetaXG   = dataMR_PECAM1toCovid_keep$BetaXG
seBetaXG = dataMR_sEselectintoCovid_keep$seBetaXG 
...
#sICAM1
 #Rename required columns
dataMR_sICAM1toCovid_keep$BetaXG<-dataMR_sICAM1toCovid_keep$beta.exposure
dataMR_sICAM1toCovid_keep$seBetaXG<-dataMR_sICAM1toCovid_keep$se.exposure
BetaXG   = dataMR_sICAM1toCovid_keep$BetaXG
seBetaXG = dataMR_sICAM1toCovid_keep$seBetaXG 
...
MR_GCNT4toCovid_keep<- mr(dataMR_GCNT4toCovid_keep, method_list = c("mr_two_sample_ml", "mr_egger_regression", "mr_simple_median", "mr_weighted_median", "mr_ivw", "mr_ivw_radial", "mr_ivw_mre", "mr_ivw_fe", "mr_simple_mode", "mr_weighted_mode"))
MR_RAB14toCovid_keep<- mr(dataMR_RAB14toCovid_keep, method_list = c("mr_two_sample_ml", "mr_egger_regression", "mr_simple_median", "mr_weighted_median", "mr_ivw", "mr_ivw_radial", "mr_ivw_mre", "mr_ivw_fe", "mr_simple_mode", "mr_weighted_mode"))
MR_C1GALT1C1toCovid_keep<- mr(dataMR_C1GALT1C1toCovid_keep, method_list = c("mr_two_sample_ml", "mr_egger_regression", "mr_simple_median", "mr_weighted_median", "mr_ivw", "mr_ivw_radial", "mr_ivw_mre", "mr_ivw_fe", "mr_simple_mode", "mr_weighted_mode"))
MR_CD207toCovid_keep<- mr(dataMR_CD207toCovid_keep, method_list = c("mr_two_sample_ml", "mr_egger_regression", "mr_simple_median", "mr_weighted_median", "mr_ivw", "mr_ivw_radial", "mr_ivw_mre", "mr_ivw_fe", "mr_simple_mode", "mr_weighted_mode"))
...
MR_sEselectintoCovid_keep<- mr(dataMR_sEselectintoCovid_keep, method_list = c("mr_two_sample_ml", "mr_egger_regression", "mr_simple_median", "mr_weighted_median", "mr_ivw", "mr_ivw_radial", "mr_ivw_mre", "mr_ivw_fe", "mr_simple_mode", "mr_weighted_mode"))
MR_SELLtoCovid_keep<- mr(dataMR_SELLtoCovid_keep, method_list = c("mr_two_sample_ml", "mr_egger_regression", "mr_simple_median", "mr_weighted_median", "mr_ivw", "mr_ivw_radial", "mr_ivw_mre", "mr_ivw_fe", "mr_simple_mode", "mr_weighted_mode"))
MR_SELEtoCovid_keep<- mr(dataMR_SELEtoCovid_keep, method_list = c("mr_two_sample_ml", "mr_egger_regression", "mr_simple_median", "mr_weighted_median", "mr_ivw", "mr_ivw_radial", "mr_ivw_mre", "mr_ivw_fe", "mr_simple_mode", "mr_weighted_mode"))
MR_sICAM1toCovid_keep<- mr(dataMR_sICAM1toCovid_keep, method_list = c("mr_two_sample_ml", "mr_egger_regression", "mr_simple_median", "mr_weighted_median", "mr_ivw", "mr_ivw_radial", "mr_ivw_mre", "mr_ivw_fe", "mr_simple_mode", "mr_weighted_mode"))
MR_SELE_1toCovid_keep<- mr(dataMR_SELE_1toCovid_keep, method_list = c("mr_two_sample_ml", "mr_egger_regression", "mr_simple_median", "mr_weighted_median", "mr_ivw", "mr_ivw_radial", "mr_ivw_mre", "mr_ivw_fe", "mr_simple_mode", "mr_weighted_mode"))
MR_SELE_2toCovid_keep<- mr(dataMR_SELE_2toCovid_keep, method_list = c("mr_two_sample_ml", "mr_egger_regression", "mr_simple_median", "mr_weighted_median", "mr_ivw", "mr_ivw_radial", "mr_ivw_mre", "mr_ivw_fe", "mr_simple_mode", "mr_weighted_mode"))
MR_PECAM1toCovid_keep<- mr(dataMR_PECAM1toCovid_keep, method_list = c("mr_two_sample_ml", "mr_egger_regression", "mr_simple_median", "mr_weighted_median", "mr_ivw", "mr_ivw_radial", "mr_ivw_mre", "mr_ivw_fe", "mr_simple_mode", "mr_weighted_mode"))
MR_PECAM_1toCovid_keep<- mr(dataMR_PECAM_1toCovid_keep, method_list = c("mr_two_sample_ml", "mr_egger_regression", "mr_simple_median", "mr_weighted_median", "mr_ivw", "mr_ivw_radial", "mr_ivw_mre", "mr_ivw_fe", "mr_simple_mode", "mr_weighted_mode"))
...
Results_MR_sEselectintoCovid_keep=MR_sEselectintoCovid_keep # Saves the results in dataframe
...
Results_MR_sICAM1toCovid_keep=MR_sICAM1toCovid_keep # Saves the results in dataframe
...
Results_MR_PECAM1toCovid_keep=MR_PECAM1toCovid_keep # Saves the results in dataframe
...
MR_sEselectintoCovid_plot = mr_scatter_plot(Results_MR_sEselectintoCovid_keep, dataMR_sEselectintoCovid_keep)
...
MR_sICAM1toCovid_plot = mr_scatter_plot(Results_MR_sICAM1toCovid_keep, dataMR_sICAM1toCovid_keep)
...
MR_PECAM1toCovid_plot = mr_scatter_plot(Results_MR_PECAM1toCovid_keep, dataMR_PECAM1toCovid_keep)
...
setwd("/scratch/groups/ukbiobank/usr/alish/Covid19/MR/A2_All/cis/")

ABO_snps  <- c("rs11244090"  ,
              "rs117322380",
              "rs183853102",
              "rs2073872",
              "rs28699275",
...
sEselectin_snps <- c("rs2142728")
sEselectin_snps <- as.data.frame(sEselectin_snps)
colnames(sEselectin_snps) <- c("SNP")

sICAM1_snps  <- c("rs11085532",
                   "rs11881066",
                   "rs144087245",
                   "rs17852402",
                   "rs1799969",
                   ...
                   "rs80113737",
                   "rs8111930")
sICAM1_snps <- as.data.frame(sICAM1_snps)
colnames(sICAM1_snps) <- c("SNP")
...
write.table(ABO, "ABO", col.names=T, row.names=F, quote=F,sep = "\t")
write.table(sEselectin, "sEselectin", col.names=T, row.names=F, quote=F,sep = "\t")
write.table(SELE, "SELE", col.names=T, row.names=F, quote=F,sep = "\t")
write.table(sICAM1, "sICAM1", col.names=T, row.names=F, quote=F,sep = "\t")
...
MR_ABOtoCovid_keep<- mr(dataMR_ABOtoCovid_keep, method_list = c("mr_two_sample_ml", "mr_egger_regression", "mr_simple_median", "mr_weighted_median", "mr_ivw", "mr_ivw_radial", "mr_ivw_mre", "mr_ivw_fe", "mr_simple_mode", "mr_weighted_mode"))
MR_sEselectintoCovid_keep<- mr(dataMR_sEselectintoCovid_keep, method_list = c("mr_two_sample_ml", "mr_egger_regression", "mr_simple_median", "mr_weighted_median", "mr_ivw", "mr_ivw_radial", "mr_ivw_mre", "mr_ivw_fe", "mr_simple_mode", "mr_weighted_mode", "mr_wald_ratio"))
